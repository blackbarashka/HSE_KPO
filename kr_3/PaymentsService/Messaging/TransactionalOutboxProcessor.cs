    ...
    public class TransactionalOutboxProcessor : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                // Задержка перед повторной обработкой
            ...
            foreach (var message in messages)
            {
                try
                {
                    ...
                    _channel.BasicPublish(
                        exchange: "",
                        routingKey: _settings.PaymentResponseQueue,
                        basicProperties: null,
                        body: body);

                    message.ProcessedAt = DateTime.UtcNow;

                    _logger.LogInformation($"Published message {message.MessageId} to {_settings.PaymentResponseQueue}");
                }
                catch (Exception ex)
                {
